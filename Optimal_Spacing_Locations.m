% Optimal_Spacing_Simulations_sparse.m
%
...
spaceFac = 1; %sqrt(2);  % gap in spacing between phosphenes.  
                     % number of electrodes is reduced by (about) a factor
                     % of sqrt(spaceFac)
maxEcc = 32;
...
while(xi(end)+si(end)<maxEcc)
    si(i) = spaceFac*(xi(i)*slope+intercept);
    xi(i+1) =(xi(i)+spaceFac*(si(i)+intercept)/2)/(1-slope/2);
    i=i+1;
end
si(i) = spaceFac*(xi(i)*slope+intercept);
...
col(id,:) = cmap(min(ceil(sz(id)*5),100),:);
...
disp(sprintf('%d electrodes',length(p.x)));
