...
for chip_x = 1:2
    for e_x = 1:4
        for e_y = 1:4
            c.e(i).radius = 1/1000; % scale is cm, if too small then the electrodes disappear due to undersampling of cortex
            c.e(i).hemi = 'lh';
            c.e(i).x = (1+(chip_x-1)*7) + ((e_x-1).*0.2);
            c.e(i).y = ((e_y-1).*0.2);
            i = i+1;
        end
    end
...
for ii=1:length(v.e) % can work with parfor
  ...
  % generates the sum of weighted receptive fields activated by an electrode
    ...
    [trl,v] = p2p_c.generate_phosphene(v, tp, trl); % multiply the rfmap by the response over time to create a percept over time
...
for k = 1:length(itpl)
    ...
    for ii=1:length(savedtrial)
        ...
        img = img + v.e(ii).rfmap(:,:,1).*savedtrial(ii).resp(itpl(k));
        end
