    ...
    if ~isempty(D(ii).condition)
        ...
        for pixNum = 1:length(c.x(:))
            ...
            if Dsim.site(ii).EF.cortex(pixNum) > 0.25
                ...
                Dsim.site(ii).EF.normalizedphosphene =  Dsim.site(ii).EF.normalizedphosphene + Dsim.site(ii).EF.cortex(pixNum) * G_scoreboard;
        ...
        gvals=logical(~isnan(D(ii).poly_area.val) & D(ii).poly_area.val>0);
        
        if ~c.smallflag
            p.thr = 0.5;  p=UWfit('findDrawingThreshold', p, {'thr'}, Dsim.site(ii).EF.normalizedphosphene(:), Dsim.site(ii).CI.val(gvals), ...
                (D(ii).poly_area.val(gvals) * c.pixperdeg^2));
            Dsim.site(ii).EF.drawingthreshold = p.thr;
            disp(['EF drawing threshold = ', num2str(Dsim.site(ii).EF.drawingthreshold)]);
            
            p.thr = 0.5; p=UWfit('findDrawingThreshold', p, {'thr'}, Dsim.site(ii).CM.normalizedphosphene(:), Dsim.site(ii).CI.val(gvals), ...
                (D(ii).poly_area.val(gvals) * c.pixperdeg^2));
            Dsim.site(ii).CM.drawingthreshold = p.thr;
            disp(['CM drawing threshold = ', num2str(Dsim.site(ii).CM.drawingthreshold)]);
        end
        for cc=1:length(D(ii).condition.val)
            Dsim.site(ii).EF.poly_area.val(cc) = (1/(c.pixperdeg^2)) * ...
                length( find((Dsim.site(ii).EF.normalizedphosphene(:) * Dsim.site(ii).CI.val(cc) )>  Dsim.site(ii).EF.drawingthreshold) );
            Dsim.site(ii).CM.poly_area.val(cc) = (1/(c.pixperdeg^2)) * ...
                length( find((Dsim.site(ii).CM.normalizedphosphene(:) * Dsim.site(ii).CI.val(cc) )>  Dsim.site(ii).CM.drawingthreshold) );
        end
...
if c.smallflag
    ...
    Dsim.site(ii).EF.drawingthreshold = .1;
    Dsim.site(ii).CM.drawingthreshold = .1;
else
