...
for ii=1:length(D) % for each site
    ...
    if ~isempty(D(ii).condition)
        ...
        gvals=find(D(ii).stimulation_data.val>0);
        if ii==5; gvals=setdiff(gvals,3:5); end % drawings not obtained
        poly_area = D(ii).poly_area.val(gvals);
        poly_area(isnan(poly_area))=0;
            
        if ~c.smallflag
            p.thr = 5;  p=UWfit('findDrawingThreshold', p, {'thr'}, Dsim(ii).EF.normalizedphosphene(:), Dsim(ii).CI.val(gvals), ...
                (poly_area * c.pixperdeg^2));
            Dsim(ii).EF.drawingthreshold = p.thr;
            disp(['EF drawing threshold = ', num2str(Dsim(ii).EF.drawingthreshold)]);
            
            p.thr = 5; p=UWfit('findDrawingThreshold', p, {'thr'}, Dsim(ii).CM.normalizedphosphene(:), Dsim(ii).CI.val(gvals), ...
                (poly_area * c.pixperdeg^2));
            Dsim(ii).CM.drawingthreshold = p.thr;
            disp(['CM drawing threshold = ', num2str(Dsim(ii).CM.drawingthreshold)]);
        end
        for cc=gvals
            Dsim(ii).EF.poly_area.val(cc) = (1/(c.pixperdeg^2)) * ...
                length( find((Dsim(ii).EF.normalizedphosphene(:) * ...
                Dsim(ii).CI.val(cc) )>  Dsim(ii).EF.drawingthreshold) );
            Dsim(ii).CM.poly_area.val(cc) = (1/(c.pixperdeg^2)) * ...
                length( find((Dsim(ii).CM.normalizedphosphene(:) * ...
                Dsim(ii).CI.val(cc) )>  Dsim(ii).CM.drawingthreshold) );
        end
