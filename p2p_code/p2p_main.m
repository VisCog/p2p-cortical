...
function v  = generate_rfmapnouse(c, v)
cosMap = cos(c.ORmap);
sinMap = sin(c.ORmap);
...
for pixNum = 1:length(c.X(:))
    ...
    if c.e.ef(pixNum) > 0.5
        ...
        theta = c.ORmap(pixNum);  %orientation
        sigma_x = c.rf.size(pixNum) * c.ar; % major axis sd
        sigma_y = c.rf.size(pixNum); % minor axis sd
        ...
        v.e.phos(:, :, 1)  =   v.e.phos(:, :, 1)  +      c.ODmap(pixNum)*G;
        v.e.phos(:, :, 2)  =   v.e.phos(:, :, 2)  + (1-c.ODmap(pixNum))*G;
        
        % scoreboard version
        v.e.phos_noRF = v.e.phos_noRF + (c.e.ef(pixNum) * exp(-( (v.X-x0).^2/(0.1) + (v.Y-y0).^2/(.1))));
