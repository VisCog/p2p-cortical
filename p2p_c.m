    ...
    methods(Static)
        ...
        function trl = generate_pt(trl, tp)
            if isnan(trl.freq)
                ...
                if trl.ip ==0
                    ...
                    if max(abs(tmp))>trl.amp
                        error('something wrong with generate_pt')
                    end
                else
        ...
        function [trl,v] = generate_phosphene(v, tp, trl, varargin)
            if isnan(trl.freq)
                ...
                if nargin==4 % short cut if already know the max response
                    trl.resp = varargin{1};
                else
        ...
        function tp = define_temporalparameters(varargin)
            ...
            if ~isfield(tp, 'simdur');   tp.simdur = 3;   end
