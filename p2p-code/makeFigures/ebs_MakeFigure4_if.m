...
Dtmp=D; % replace with cortical simulation
for ii = 1:length(D)
    Dtmp(ii).poly_area.val = Dsim(ii).A(1).chargedensity.val;
end
opts.titlestr = 'phosphene 4a';
figure4a(Dtmp,  opts);
...
c.thr = 0.25; % the level of cortical response that is considered above perceptual threshold
