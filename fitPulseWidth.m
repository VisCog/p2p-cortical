...
for i = 1:length(paperList)
    for j=1:length(exList{i})
        ...
        y = Texp.amp*scFac;
        figure(1)
        h(count)= plot(log(Texp.pw*1000),y,'o', 'MarkerFaceColor', colList{count}, 'MarkerEdgeColor','none','MarkerSize',sz);
        drawnow
        
        figure(2)
        hold on
        scFac2 = standard_thresh'/thresh';
        plot(log(Texp.pw*1000),thresh*scFac2,'o', 'MarkerFaceColor', colList{count}, 'MarkerEdgeColor','none','MarkerSize',sz);
        drawnow
